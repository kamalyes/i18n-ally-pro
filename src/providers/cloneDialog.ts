...
export function getCloneDialogCss(): string {
  return `
    .clone-overlay { display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); z-index: 100; justify-content: center; align-items: center; }
    .clone-overlay.show { display: flex; }
    .clone-dialog { background: #252526; border: 1px solid #4CAF50; border-radius: 8px; padding: 24px 28px; min-width: 560px; max-width: 720px; max-height: 80vh; overflow-y: auto; box-shadow: 0 8px 32px rgba(0,0,0,0.5); }
    .clone-dialog h3 { color: #4CAF50; margin-bottom: 16px; font-size: 16px; font-weight: 600; }
    .clone-dialog .clone-label { color: #aaa; font-size: 13px; display: block; margin-bottom: 6px; }
    .clone-dialog select { width: 100%; padding: 8px 12px; background: #2d2d2d; border: 1px solid #555; border-radius: 4px; color: #d4d4d4; font-size: 13px; margin-bottom: 16px; outline: none; }
    .clone-dialog select:focus { border-color: #4CAF50; }
    .clone-target-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; margin-bottom: 16px; max-height: 320px; overflow-y: auto; padding: 4px; }
    .clone-target-item { display: flex; align-items: center; gap: 6px; padding: 8px 6px; background: #2d2d2d; border: 1px solid #444; border-radius: 4px; cursor: pointer; font-size: 12px; transition: all 0.15s; }
    .clone-target-item:hover { border-color: #4CAF50; background: #3d3d3d; }
    .clone-target-item.selected { border-color: #4CAF50; background: rgba(76,175,80,0.15); }
    .clone-target-item.exists { opacity: 0.5; }
    .clone-target-item .flag-icon-wrap { width: 22px; height: 16px; border-radius: 1px; overflow: hidden; flex-shrink: 0; border: 1px solid rgba(255,255,255,0.08); }
    .clone-target-item .flag-icon-wrap .fi { width: 22px; height: 16px; display: block; background-size: contain; background-position: center; background-repeat: no-repeat; }
    .clone-target-item .locale-code { color: #d4d4d4; }
    ...
    .clone-dialog .clone-checkbox { display: flex; align-items: center; gap: 8px; margin-bottom: 16px; color: #aaa; font-size: 12px; }
    ...
    .clone-dialog .btn-cancel { padding: 6px 18px; background: #2d2d2d; border: 1px solid #555; border-radius: 4px; color: #d4d4d4; cursor: pointer; font-size: 13px; }
    .clone-dialog .btn-cancel:hover { background: #3d3d3d; }
    .clone-dialog .btn-confirm { padding: 6px 18px; background: rgba(76,175,80,0.15); border: 1px solid #4CAF50; border-radius: 4px; color: #4CAF50; cursor: pointer; font-size: 13px; }
